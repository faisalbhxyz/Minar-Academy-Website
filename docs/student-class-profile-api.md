...
**Related:** [free-lessons-api.md](./free-lessons-api.md) · [backend-api.md](./backend-api.md)
...
## App wiring (mobile — after this API)

| Current | After |
| --- | --- |
| `saveOnboardingProfile()` AsyncStorage only | Also `PUT /student/class-profile` |
| `hydrate(userId)` from AsyncStorage | Prefer `user.class_profile` from login, then details |
| `saveSelectedClassSlug()` local only | Also `PUT` `{ preferred_class_slug }` |
| Free lessons / home filter | Use server `preferred_class_slug` when logged in |

Files: `onboardingStore.ts`, `storage.ts`, `api/index.ts`, `types/api.ts` (`Student.class_profile`), onboarding complete path, `HomeScreen` class picker.
